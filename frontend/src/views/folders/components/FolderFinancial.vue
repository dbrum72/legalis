    ...
    <section class="folder-financial">
        <header class="folder-financial__header">
            ...
            <div class="folder-financial__actions">
                <AppButton v-if="canGenerateBilling" size="sm" variant="navigation" @click="toggleForm('billing')">Gerar cobrança</AppButton>
        ...
        <form v-if="activeForm === 'billing'" class="folder-financial__form" @submit.prevent="submitBilling">
            <div><h3>Gerar cobrança dos lançamentos</h3><p>Selecione horas e despesas que serão encerradas como faturadas.</p></div>
            <div class="folder-financial__grid">
                <label>Cliente<select v-model="billingForm.client_id" required><option value="" disabled>Selecione</option><option v-for="client in clients" :key="client.id" :value="client.id">{{ client.name }}</option></select></label>
                <label>Contrato<select v-model="billingForm.fee_agreement_id"><option value="">Sem contrato</option><option v-for="agreement in activeAgreements" :key="agreement.id" :value="agreement.id">{{ agreement.client?.name }} · {{ agreementLabel(agreement) }}</option></select></label>
                <label>Vencimento<input v-model="billingForm.due_on" type="date" required></label>
                <AppCurrency v-model="billingForm.discount" id="billing-discount" label="Desconto" :min="0" :max="billingSubtotal / 100" :allow-empty="false" shift-decimal />
            </div>
            <fieldset class="folder-financial__billing-items"><legend>Horas faturáveis</legend><label v-for="entry in billableEntries" :key="entry.id"><input v-model="billingForm.time_entry_ids" type="checkbox" :value="entry.id"><span><strong>{{ entry.description }}</strong><small>{{ formatDate(entry.worked_on) }} · {{ formatDuration(entry.duration_minutes) }}</small></span><b>{{ formatMoney(entryAmount(entry)) }}</b></label><p v-if="!billableEntries.length">Nenhuma hora disponível.</p></fieldset>
            <fieldset class="folder-financial__billing-items"><legend>Despesas reembolsáveis</legend><label v-for="expense in billableExpenses" :key="expense.id"><input v-model="billingForm.expense_ids" type="checkbox" :value="expense.id"><span><strong>{{ expense.description }}</strong><small>{{ formatDate(expense.incurred_on) }}</small></span><b>{{ formatMoney(expense.amount_cents) }}</b></label><p v-if="!billableExpenses.length">Nenhuma despesa disponível.</p></fieldset>
            <div class="folder-financial__billing-total"><span>Total selecionado</span><strong>{{ formatMoney(billingTotal) }}</strong></div>
            <div class="folder-financial__form-actions"><AppButton type="button" variant="ghost" @click="closeForm">Cancelar</AppButton><AppButton type="submit" :loading="submitting" :disabled="billingSubtotal < 1">Gerar cobrança</AppButton></div>
        </form>
        ...
        <form v-if="activeForm === 'time'" class="folder-financial__form" @submit.prevent="submitTime">
            ...
            <div class="folder-financial__form-actions"><AppButton type="button" variant="ghost" @click="closeForm">Cancelar</AppButton><AppButton type="submit" :loading="submitting">{{ editingTime ? 'Salvar alterações' : 'Salvar apontamento' }}</AppButton></div>
        </form>
        ...
        <form v-if="activeForm === 'expense'" class="folder-financial__form" @submit.prevent="submitExpense">
            ...
            <div class="folder-financial__grid">
                ...
                <AppCurrency v-model="expenseForm.amount" id="expense-amount" label="Valor" :min="0.01" :allow-empty="false" shift-decimal required />
            ...
            <div class="folder-financial__form-actions"><AppButton type="button" variant="ghost" @click="closeForm">Cancelar</AppButton><AppButton type="submit" :loading="submitting">{{ editingExpense ? 'Salvar alterações' : 'Salvar despesa' }}</AppButton></div>
        </form>
        ...
        <section v-if="canViewFinance" class="folder-financial__block">
            ...
            <article v-for="agreement in financialStore.agreements" :key="agreement.id" class="folder-financial__row">
                <div><strong>{{ agreement.client?.name ?? 'Sem cliente' }}</strong><span>{{ agreementLabel(agreement) }}</span><small v-if="agreement.starts_on || agreement.ends_on">Vigência: {{ formatDate(agreement.starts_on) }} a {{ formatDate(agreement.ends_on) }}</small></div>
        ...
        <section v-if="canViewTime" class="folder-financial__block">
            ...
            <article v-for="entry in financialStore.timeEntries" :key="entry.id" class="folder-financial__row">
                ...
                <div v-if="entry.status === 'open'" class="folder-financial__row-actions"><AppButton v-if="canUpdateTime" size="sm" variant="outline" @click="editTime(entry)">Editar</AppButton><AppButton v-if="canDeleteTime" size="sm" variant="ghost" @click="askDelete('time', entry)">Excluir</AppButton></div>
            </article>
        </section>
        ...
        <section v-if="canViewExpenses" class="folder-financial__block">
            ...
            <article v-for="expense in financialStore.expenses" :key="expense.id" class="folder-financial__row">
                ...
                <div v-if="expense.status === 'open'" class="folder-financial__row-actions"><AppButton v-if="canUpdateExpense" size="sm" variant="outline" @click="editExpense(expense)">Editar</AppButton><AppButton v-if="canDeleteExpense" size="sm" variant="ghost" @click="askDelete('expense', expense)">Excluir</AppButton></div>
            </article>
        </section>
...
const expenseForm = reactive({ incurred_on: today(), amount: 0, description: '', reimbursable: true })
const billingForm = reactive({ client_id: '', fee_agreement_id: '', due_on: today(), discount: 0, time_entry_ids: [], expense_ids: [] })
...
const billableEntries = computed(() => financialStore.timeEntries.filter((entry) => entry.billable && entry.status === 'open' && !entry.invoice_id))
const billableExpenses = computed(() => financialStore.expenses.filter((expense) => expense.reimbursable && expense.status === 'open' && !expense.invoice_id))
const activeAgreements = computed(() => financialStore.agreements.filter((agreement) => agreement.status === 'active'))
const canGenerateBilling = computed(() => canManageFinance.value && (billableEntries.value.length > 0 || billableExpenses.value.length > 0))
const entryAmount = (entry) => Math.round(Number(entry.duration_minutes || 0) * Number(entry.hourly_rate_cents || 0) / 60)
const billingSubtotal = computed(() => billableEntries.value.filter((entry) => billingForm.time_entry_ids.includes(entry.id)).reduce((total, entry) => total + entryAmount(entry), 0) + billableExpenses.value.filter((expense) => billingForm.expense_ids.includes(expense.id)).reduce((total, expense) => total + Number(expense.amount_cents), 0))
const billingTotal = computed(() => Math.max(0, billingSubtotal.value - Math.round(Number(billingForm.discount || 0) * 100)))
...
function agreementLabel(item) { const terms = []; if (item.hourly_rate_cents) terms.push(`${formatMoney(item.hourly_rate_cents)}/hora`); if (item.fixed_fee_cents) terms.push(formatMoney(item.fixed_fee_cents)); if (item.contingency_percentage) terms.push(`${item.contingency_percentage}% de êxito`); return terms.join(' + ') || item.type }
...
function resetTimeForm() { Object.assign(timeForm, { worked_on: today(), duration_minutes: 60, description: '', billable: true }); editingTime.value = null }
function resetExpenseForm() { Object.assign(expenseForm, { incurred_on: today(), amount: 0, description: '', reimbursable: true }); editingExpense.value = null }
function resetBillingForm() { Object.assign(billingForm, { client_id: props.clients[0]?.id ?? '', fee_agreement_id: '', due_on: today(), discount: 0, time_entry_ids: billableEntries.value.map((entry) => entry.id), expense_ids: billableExpenses.value.map((expense) => expense.id) }) }
function closeForm() { activeForm.value = ''; editingAgreement.value = null; editingTime.value = null; editingExpense.value = null }
function toggleForm(name) { if (activeForm.value === name) { closeForm(); return } if (name === 'agreement') resetAgreementForm(); if (name === 'time') resetTimeForm(); if (name === 'expense') resetExpenseForm(); if (name === 'billing') resetBillingForm(); activeForm.value = name; error.value = ''; success.value = '' }
...
function editTime(entry) { editingTime.value = entry; Object.assign(timeForm, { worked_on: String(entry.worked_on).slice(0, 10), duration_minutes: entry.duration_minutes, description: entry.description, billable: Boolean(entry.billable) }); activeForm.value = 'time'; error.value = '' }
function editExpense(expense) { editingExpense.value = expense; Object.assign(expenseForm, { incurred_on: String(expense.incurred_on).slice(0, 10), amount: Number(expense.amount_cents) / 100, description: expense.description, reimbursable: Boolean(expense.reimbursable) }); activeForm.value = 'expense'; error.value = '' }
...
function submitTime() { const payload = { ...timeForm }; return submit(() => editingTime.value ? financialStore.updateTime(props.folderId, editingTime.value.id, payload) : financialStore.createTime(props.folderId, payload)) }
function submitExpense() { const payload = { incurred_on: expenseForm.incurred_on, description: expenseForm.description, amount_cents: Math.round(expenseForm.amount * 100), reimbursable: expenseForm.reimbursable }; return submit(() => editingExpense.value ? financialStore.updateExpense(props.folderId, editingExpense.value.id, payload) : financialStore.createExpense(props.folderId, payload)) }
async function submitBilling() { submitting.value = true; error.value = ''; success.value = ''; try { const invoice = await financialStore.createBilling(props.folderId, { client_id: Number(billingForm.client_id), fee_agreement_id: billingForm.fee_agreement_id ? Number(billingForm.fee_agreement_id) : null, due_on: billingForm.due_on, discount_cents: Math.round(Number(billingForm.discount || 0) * 100), time_entry_ids: billingForm.time_entry_ids, expense_ids: billingForm.expense_ids }); closeForm(); success.value = `Cobrança ${invoice.charge_identifier} gerada com sucesso.` } catch (exception) { error.value = exception?.response?.data?.message ?? Object.values(exception?.response?.data?.errors ?? {})[0]?.[0] ?? 'Não foi possível gerar a cobrança.' } finally { submitting.value = false } }
...
watch(() => billingForm.fee_agreement_id, (id) => { const agreement = activeAgreements.value.find((item) => Number(item.id) === Number(id)); if (agreement?.client_id) billingForm.client_id = agreement.client_id })
...
.folder-financial__grid input, .folder-financial__grid select, .folder-financial__grid textarea { min-height: 2.65rem; padding: var(--space-2) var(--space-3); color: var(--color-text); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-md); font: inherit; }
...
.folder-financial__row small { color: var(--color-text-muted); }
.folder-financial__row-actions { flex-direction: row !important; gap: var(--space-2) !important; }
...
.folder-financial__success { padding: var(--space-3); color: var(--color-success); background: var(--color-success-soft); border-radius: var(--radius-md); }
.folder-financial__billing-items { display: flex; padding: 0; flex-direction: column; border: 1px solid var(--color-border); border-radius: var(--radius-md); }
.folder-financial__billing-items legend { margin-left: var(--space-3); padding: 0 var(--space-2); color: var(--color-text-soft); font-weight: 700; }
.folder-financial__billing-items > label { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; padding: var(--space-3) var(--space-4); align-items: center; gap: var(--space-3); border-bottom: 1px solid var(--color-border); cursor: pointer; }
.folder-financial__billing-items > label:last-of-type { border-bottom: 0; }
.folder-financial__billing-items > label span { display: flex; min-width: 0; flex-direction: column; }
.folder-financial__billing-items small, .folder-financial__billing-items > p { color: var(--color-text-muted); }
.folder-financial__billing-items > p { margin: 0; padding: var(--space-4); }
.folder-financial__billing-total { display: flex; align-items: center; justify-content: flex-end; gap: var(--space-3); }
.folder-financial__billing-total strong { color: var(--color-brand-secondary); font-size: 1.4rem; }
